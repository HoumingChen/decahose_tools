...
class Tools():
    ...
    def ls(self):
        ...
        files_var = subprocess.check_output(cmd_var.split(' '), shell=True).decode().strip().split('\n')
        cmd_data = 'hdfs dfs -ls /data/twitter/decahose/'
        files_data = subprocess.check_output(cmd_data.split(' '), shell=True).decode().strip().split('\n')
